...
def test_simple_exp(Calculator_Eval):
    test = (
        ...
        ("varp([12.5, 13, 18, 11.8, 2.3, 7])", 24.5422222222222),
        ("var([9.0, 1.0, 1.0, 1.0, 0.0, 0.0, 4.0, 16.0])", 32.5714285714286),
        ("stdevp([0.2, 0.7, .4,-.3,-.7])", 0.5003998401279),
        ("stdev([999,888.7,777.2,-666.4,2])", 713.3585073439582),

        ("5*6+81/23mod 74*2^8", 931.5652173913044),
        ("2 * 3^2 + 2 / 3mod 2 - 1", 19),
        ("-.12345+54321*123/321^2mod(sum([-1+10^2*rsin(rcos(rtan(100))),rarcsin(-1+1-1),log(100,10)*log10(1000)/ln(exp(1)),cuberoot(sqrt(fact(10)*mod(5,3)))*yroot(5,2),avg([3,4,5,6]),sum([987,253])*varp([1,11,111,1111])/stdevp([123,321,456,654,789,987])]))", 64.71950571665647),
        ...
        ("sum([-(-1)--(2+3),3])", 9),
        ("sum([-1--2,3])", 4),
        ("stdevp([1,2,sin(90)])+2", 2.471404520791),
...
def test_equation(Calculator_Xrun):
    case = (
        ...
        ("-.12345+54321*$$/321^2mod(sum([-1+10^2*rsin(rcos(rtan(100))),rarcsin(-1+1-1),log(100,10)*log10(1000)/ln(exp(1)),cuberoot(sqrt(fact(10)*mod(5,3)))*yroot(5,2),avg([3,4,5,6]),sum([987,253])*varp([1,11,111,1111])/stdevp([123,321,456,654,789,987])])) = 64.71950571665647", "123"),
    )
    for a,b in case:
...
def test_function(Calculator_Xrun):
    case1 = (
        ("#func:-.12345+$5*123/321^2mod(sum([-1+$1^2*rsin(rcos(rtan($1))),rarcsin(-1+1-1),log($3,$2)*log10($4)/ln(exp($1)),cuberoot(sqrt(fact($2)*mod(5,3)))*yroot(5,2),avg([3,4,5,6]),sum([987,253])*varp([$1,11,111,1111])/stdevp([123,321,456,654,789,987])]))", "#f:1+1"),
